...
export const FlightDetails = ({ route, airline }: { route: Route; airline: string }) => {
  ...
  const getScaleDetails = () => {
    if(!route.totalStopDuration) { return "" }
    if(route.stopNumber === 0) { return "" }
    if(route.stopNumber === 1) { return "1 escale (" + transformDuration(route.totalStopDuration) + ")" }
    if(route.stopNumber >= 2 ) { return route.stopNumber + " escales (" + transformDuration(route.totalStopDuration) + ")" }
  }
    ...
    <Stack gap={4} direction="row">
      ...
      <Stack flexGrow={1}>
        <Stack direction="row" justifyContent="space-between" gap={2}>
          ...
          {numberDaysArrival > 0 && (<Typography variant="bodySm" color="grey.700">
            J+{numberDaysArrival}
          </Typography>)}
          <Typography variant="titleLg" color="leclerc.red.main">
            {arrivalTime}
        ...
        <Stack direction="row" gap={4.5}>
          ...
          <Stack textAlign="center" width="40%" gap="2px">
            ...
            {route.airportChange && (<Typography variant="bodySm" color="leclerc.red.main">
        ...
        <Stack direction="row" justifyContent="space-between" pt={1} alignItems="center">
          ...
          {false && <Typography variant="bodySm" color="grey.700">
            {carbonFootprint}
          </Typography>}
        </Stack>
      </Stack>
